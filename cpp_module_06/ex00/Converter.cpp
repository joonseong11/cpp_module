...
void Converter::toChar(void) {
	...
	{
		std::cout << "char: " 
			<< static_cast<char>(double_data)<< std::endl;
		return ;
	}
...
void Converter::toInt(void) {
	if (double_data > INT_MAX || double_data < INT_MIN 
		|| double_data == INFINITY || isnan(double_data))
	{
		std::cout << "int: " 
			<< "impossible" << std::endl;
	}
		...
		std::cout << "int: " 
			<< static_cast<int>(double_data)<< std::endl;
}

void Converter::toFloat(void) {
	std::cout << "float: " << static_cast<float>(double_data);
	if (double_data == floor(double_data) && double_data < 1000000 
		&& double_data > -1000000)
	{
		std::cout << ".0";
	}
